                        ...
                        if (y2) {
                          ...
                          if (o3 && (e5 = y2.body.classList).add.apply(e5, (0, t2.__spreadArray)([], (0, t2.__read)(o3.split(" ")), false)), !h) {
                            ...
                            for (O = 0; O < b.length; O++) {
                              var T = b[O], j = T.getAttribute("src");
                              j ? ((k = new Image()).onload = v.bind(null, T, true), k.onerror = v.bind(null, T, false), k.src = j) : (n3.logMessages(['"react-to-print" encountered an <img> tag with an empty "src" attribute. It will not attempt to pre-load it. The <img> is:', T], "warning"), v(T, false));
                            }
                            for (O = 0; O < g.length; O++) {
                              var A = g[O];
                              A.preload = "auto";
                              var k, E = A.getAttribute("poster");
                              E ? ((k = new Image()).onload = v.bind(null, A, true), k.onerror = v.bind(null, A, false), k.src = E) : A.readyState >= 2 ? v(A, true) : (A.onloadeddata = v.bind(null, A, true), A.onerror = v.bind(null, A, false), A.onstalled = v.bind(null, A, false));
                            }
                          ...
                          if (a2)
                            for (var N = document.querySelectorAll("style, link[rel='stylesheet']"), B = (O = 0, N.length); O < B; ++O) {
                              ...
                              if ("style" === G.tagName.toLowerCase()) {
                                var Y = y2.createElement(G.tagName), z = G.sheet;
                                if (z) {
                                  var H = "";
                                  try {
                                    for (var V = z.cssRules.length, U = 0; U < V; ++U)
                                      "string" == typeof z.cssRules[U].cssText && (H += "".concat(z.cssRules[U].cssText, "\r\n"));
                                  } catch (e6) {
                                  ...
                                  Y.setAttribute("id", "react-to-print-".concat(O)), u2 && Y.setAttribute("nonce", u2), Y.appendChild(y2.createTextNode(H)), y2.head.appendChild(Y);
                                }
                              } else if (G.getAttribute("href"))
                                if (G.hasAttribute("disabled"))
                                  n3.logMessages(["`react-to-print` encountered a <link> tag with a `disabled` attribute and will ignore it. Note that the `disabled` attribute is deprecated, and some browsers ignore it. You should stop using it. https://developer.mozilla.org/en-US/docs/Web/HTML/Element/link#attr-disabled. The <link> is:", G], "warning"), v(G, true);
                                else {
                                  Y = y2.createElement(G.tagName), U = 0;
                                  for (var J = G.attributes.length; U < J; ++U) {
                                    var K = G.attributes[U];
                                    K && Y.setAttribute(K.nodeName, K.nodeValue || "");
                                  }
                                  Y.onload = v.bind(null, Y, true), Y.onerror = v.bind(null, Y, false), u2 && Y.setAttribute("nonce", u2), y2.head.appendChild(Y);
                                }
                              else
                                n3.logMessages(["`react-to-print` encountered a <link> tag with an empty `href` attribute. In addition to being invalid HTML, this can cause problems in many browsers, and so the <link> was not loaded. The <link> is:", G], "warning"), v(G, true);
                            }
                        }
